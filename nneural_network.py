...
#            
    ...
    def _l_train_op(self):
        ...
        v_loss = tf.reduce_mean(tf.square(self.winner_reshape - self.value))
        c_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=self.policy_net_out, labels=self.mcts_probs)
        p_loss = tf.reduce_mean(c_entropy)
        self.loss =  v_loss + p_loss + self.l2_const*l2_penalty
    ...
    def u_p_value(self, state_batch):
         
        action_probs, value = self.sess.run([self.action_probs, self.value],
                                    feed_dict={self.state_input: state_batch})       
        return action_probs, value

    def pv_fn(self, board):
        """
        input: board
        output: a list of (action, probability) tuples for each available action and the score of the board state
        """
        l_positions = board.availables
        c_state = board.current_state()
        act_probs, value = self.sess.run([self.action_probs, self.value], 
                                    feed_dict={self.state_input: c_state.reshape(-1, 4, self.board_width, self.board_height)})
        act_probs = zip(l_positions, act_probs.flatten()[l_positions])
